...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Edit Data Pasien</h1>
            <p class="text-sm text-slate-500 mt-1">Perbarui data rekam medis pasien ID: <strong class="font-mono text-teal-700">{{ $pasien->id_pasien }}</strong></p>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8">
        <form action="{{ route('admin.patients.update', $pasien->id_pasien) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="p-5 rounded-2xl bg-teal-50/60 border border-teal-100 space-y-3">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <label class="flex items-center gap-3 p-3.5 rounded-xl bg-white border border-slate-200 cursor-pointer hover:border-teal-500 transition-all">
                        <input type="radio" name="jenis_pasien" value="Umum/Mandiri" onclick="toggleBpjsInput(false)" {{ old('jenis_pasien', $pasien->jenis_pasien) === 'Umum/Mandiri' ? 'checked' : '' }} class="text-teal-600 focus:ring-teal-500">
                        <div>
                    ...
                    <label class="flex items-center gap-3 p-3.5 rounded-xl bg-white border border-slate-200 cursor-pointer hover:border-teal-500 transition-all">
                        <input type="radio" name="jenis_pasien" value="BPJS" onclick="toggleBpjsInput(true)" {{ old('jenis_pasien', $pasien->jenis_pasien) === 'BPJS' ? 'checked' : '' }} class="text-teal-600 focus:ring-teal-500">
                        <div>
            ...
            <div id="bpjsContainer" class="{{ old('jenis_pasien', $pasien->jenis_pasien) === 'BPJS' ? '' : 'hidden' }}">
                ...
                <input type="text" name="no_bpjs" id="no_bpjs" value="{{ old('no_bpjs', $pasien->no_bpjs) }}" placeholder="13 Digit Nomor BPJS" class="w-full px-4 py-3 rounded-2xl bg-white border border-slate-300 text-slate-900 text-sm font-medium focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition-all @error('no_bpjs') border-red-500 @enderror">
                @error('no_bpjs')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="nik" id="nik" value="{{ old('nik', $pasien->nik) }}" placeholder="16 Digit NIK KTP" maxlength="16" required class="w-full px-4 py-3 rounded-2xl bg-white border border-slate-300 text-slate-900 text-sm font-medium focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition-all @error('nik') border-red-500 @enderror">
                    @error('nik')
                        <p class="mt-1.5 text-xs text-red-500 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="nama_lengkap" id="nama_lengkap" value="{{ old('nama_lengkap', $pasien->nama_lengkap) }}" placeholder="Nama sesuai KTP" required class="w-full px-4 py-3 rounded-2xl bg-white border border-slate-300 text-slate-900 text-sm font-medium focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition-all @error('nama_lengkap') border-red-500 @enderror">
                    @error('nama_lengkap')
                        <p class="mt-1.5 text-xs text-red-500 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="tempat_lahir" id="tempat_lahir" value="{{ old('tempat_lahir', $pasien->tempat_lahir) }}" placeholder="Kota Kelahiran" required class="w-full px-4 py-3 rounded-2xl bg-white border border-slate-300 text-slate-900 text-sm font-medium focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition-all @error('tempat_lahir') border-red-500 @enderror">
                    @error('tempat_lahir')
                        <p class="mt-1.5 text-xs text-red-500 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <select name="golongan_darah" id="golongan_darah" required class="w-full px-4 py-3 rounded-2xl bg-white border border-slate-300 text-slate-900 text-sm font-medium focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition-all @error('golongan_darah') border-red-500 @enderror">
                        <option value="A" {{ old('golongan_darah', $pasien->golongan_darah) === 'A' ? 'selected' : '' }}>Golongan Darah A</option>
                        <option value="B" {{ old('golongan_darah', $pasien->golongan_darah) === 'B' ? 'selected' : '' }}>Golongan Darah B</option>
                        <option value="AB" {{ old('golongan_darah', $pasien->golongan_darah) === 'AB' ? 'selected' : '' }}>Golongan Darah AB</option>
                        <option value="O" {{ old('golongan_darah', $pasien->golongan_darah) === 'O' ? 'selected' : '' }}>Golongan Darah O</option>
                        <option value="-" {{ old('golongan_darah', $pasien->golongan_darah) === '-' ? 'selected' : '' }}>Belum Tahu (-)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="nomor_telepon" id="nomor_telepon" value="{{ old('nomor_telepon', $pasien->nomor_telepon) }}" placeholder="08xxxxxxxxxx" required class="w-full px-4 py-3 rounded-2xl bg-white border border-slate-300 text-slate-900 text-sm font-medium focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition-all @error('nomor_telepon') border-red-500 @enderror">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="alamat_lengkap" id="alamat_lengkap" rows="3" placeholder="Alamat rumah lengkap" required class="w-full px-4 py-3 rounded-2xl bg-white border border-slate-300 text-slate-900 text-sm font-medium focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition-all @error('alamat_lengkap') border-red-500 @enderror">{{ old('alamat_lengkap', $pasien->alamat_lengkap) }}</textarea>
            </div>

            <!-- Password Opsional -->
            <div class="pt-4 border-t border-slate-100">
                <label for="password" class="block text-xs font-bold text-slate-700 mb-1">Ganti Password Pasien (Opsional)</label>
                <input type="password" name="password" id="password" placeholder="Kosongkan jika tidak ingin mengubah password" class="w-full px-4 py-3 rounded-2xl bg-white border border-slate-300 text-slate-900 text-sm font-medium focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition-all @error('password') border-red-500 @enderror">
                @error('password')
