...
<head>
    ...
    <title>Tiket Antrean Loket {{ $antrean->kode_antrean ?? ('#' . $antrean->nomor_antrean) }} - Klinik Nomor Satu</title>
...
<body class="bg-slate-100 font-sans text-slate-800 antialiased p-4 sm:p-8 min-h-screen flex flex-col items-center justify-center">
    ...
    <div class="no-print mb-6 flex gap-3 max-w-sm w-full">
        <a href="{{ route('admin.antrean-kunjungan.index') }}" class="flex-1 py-3 px-4 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded-2xl text-xs text-center transition-all">
            &larr; Kembali
        </a>
    ...
    <div class="print-card bg-white p-6 rounded-3xl shadow-xl border border-slate-200 max-w-sm w-full text-center space-y-4">
        ...
        <div class="border-b border-dashed border-slate-300 pb-3">
            ...
            <span class="inline-block mt-2 px-2.5 py-0.5 rounded-full bg-teal-50 text-teal-700 text-[10px] font-bold border border-teal-200">
                TIKET ANTREAN RESEPSIONIS / LOKET
            </span>
        </div>
        ...
        <div class="py-2 space-y-1">
            ...
            <p class="text-xs font-bold text-slate-600">
                Jalur: <span class="text-teal-700">{{ $antrean->jenis_pasien }}</span>
            </p>
        </div>
        ...
        <div class="border-t border-b border-dashed border-slate-300 py-3 space-y-1.5 text-xs text-left text-slate-600">
            <div class="flex justify-between">
                ...
                <span class="font-bold text-slate-900 truncate max-w-[180px]">{{ $antrean->nama_pasien }}</span>
            </div>
            <div class="flex justify-between">
                <span class="text-slate-400">NIK / ID:</span>
                <span class="font-mono text-slate-800">{{ $antrean->nik }}</span>
            </div>
            <div class="flex justify-between">
                <span class="text-slate-400">Poli Tujuan:</span>
                <span class="font-bold text-slate-900">{{ $antrean->nama_poli_master ?? $antrean->nama_poli_jadwal ?? 'Poli Umum' }}</span>
            </div>
            <div class="flex justify-between">
                <span class="text-slate-400">Dokter:</span>
                <span class="font-bold text-slate-900">Dr. {{ $antrean->nama_dokter ?? '-' }}</span>
            </div>
            <div class="flex justify-between">
        ...
        <div class="pt-1 text-[10px] text-slate-400 space-y-1">
            <p>Silakan menunggu di ruang tunggu Poliklinik.</p>
            <p class="font-mono text-[9px] text-slate-300">Dicetak Resepsionis: {{ date('d-m-Y H:i:s') }} WIB</p>
        </div>
    </div>
